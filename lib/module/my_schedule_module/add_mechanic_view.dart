...
class AddMechanicView extends StatelessWidget {
  const AddMechanicView({super.key});
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4.h),
                  Text(
                    'Select a mechanic and date to add it to your schedule.',
                  ...
                  DropDownSearchWidget<String>(
                    ...
                    selectedWidget: Text(
                      context.select((MyScheduleBloc value) =>
                              value.selectedMechanic != null
                                  ? MyScheduleBloc.mechanicLabel(
                                      value.selectedMechanic!)
                                  : null) ??
                          "Please select mechanic",
                      style: Theme.of(context).textTheme.titleSmall,
                      overflow: TextOverflow.ellipsis,
                    ),
                    enabled: true,
                    options: bloc.mechanicList
                        .map(MyScheduleBloc.mechanicLabel)
                        .toList(),
                    hint: "Please select mechanic",
                    selectedVal: context.select((MyScheduleBloc value) =>
                        value.selectedMechanic != null
                            ? MyScheduleBloc.mechanicLabel(
                                value.selectedMechanic!)
                            : null),
                    valChanged: (value) {
                      if (value != null) {
                        bloc.selectedMechanic = bloc.mechanicList.firstWhere(
                            (element) =>
                                MyScheduleBloc.mechanicLabel(element) ==
                                value);
